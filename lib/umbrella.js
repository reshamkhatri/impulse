...
export function initUmbrella(host) {
  ...
  teardown.push(() => {
    observer.disconnect();
    // Free the GPU side explicitly — a dropped WebGL context is not collected
    // promptly, and browsers cap how many may exist at once.
    scene.traverse((obj) => {
  ...
  /* rootMargin gives it a screen of runway, so it's already spinning by the
     time the section is genuinely in view. */
