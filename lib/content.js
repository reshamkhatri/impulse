...
/** Published posts, newest first. Falls back to the articles carried over from
    the old site, so /blog is never empty because of missing keys or an outage. */
export const getAllPosts = cache(async () => {
  const supabase = publicClient();
  if (!supabase) return FALLBACK_POSTS.map(toPost);
  ...
  if (error || !data?.length) return FALLBACK_POSTS.map(toPost);
  return data.map(toPost);
});

/* A miss defers to getAllPosts rather than to FALLBACK_POSTS directly. Both
   readers then agree on which set is live: once the database is serving posts,
   a slug that isn't in it is a genuine 404 and doesn't resurrect from here. */
export const getPost = cache(async (slug) => {
  const fromListing = async () => (await getAllPosts()).find((p) => p.slug === slug) ?? null;

  const supabase = publicClient();
  if (!supabase) return fromListing();
  ...
  if (error || !data) return fromListing();
  return toPost(data);
});
